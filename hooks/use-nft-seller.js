...
export function useNftSeller() {
    ...
    const saveContractInfo = useCallback(async ({ tokenId, applicationId, contractInfo, price }) => {
    ...
    const buy = useCallback(async (tokenId, applicationId, contractInfo) => {
    ...
    const withdraw = useCallback(async (tokenId, applicationId, contractInfo) => {
        if (contractInfo && walletAccount) {
            const infoObject = JSON.parse(Buffer.from(contractInfo, 'base64'))
            contract.current = walletAccount.contract(backend, infoObject)
